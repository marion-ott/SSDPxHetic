...
const Pagination = ({ totalRecords, itemPerPage, pageNeighbours, handlePageChange }) => {
  ...
  const fetchPageNumbers = () => {
    ...
    if (totalPages.current > totalBlocks) {
      ...
      const endPage = Math.min(totalPages.current - 1, currentPage.current + pageNeighbours);
        ...
        {pages && pages.map((page, index) => {
          ...
          if (page === LEFT_PAGE) return (
            <li key={index} className="page-item">
              <a className="pagination-link" href="#" aria-label="Previous" onClick={handleMoveLeft}>
          ...
          if (page === RIGHT_PAGE) return (
            <li key={index} className="page-item">
              <a className="pagination-link" href="#" aria-label="Next" onClick={handleMoveRight}>
          ...
          return (
            <li key={index} className={`page-item${currentPage.current === page ? ' active' : ''}`}>
              <a className="pagination-link" href="#" onClick={handleClick(page)}>{page}</a>
            </li>
          )
